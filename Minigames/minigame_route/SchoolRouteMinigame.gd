...
@export var board_piece_percentage: float = 0.90
...
func _calculate_board_piece_size() -> void:
	for child: Node in _slots_root.get_children():
		...
		var slot_size: Vector2 = (
			rectangle.size * scale_size
		)

		_board_piece_size = (
			slot_size * board_piece_percentage
		)
...
func _on_palette_piece_drag_requested(
	...
	var source_texture: Texture2D = (
		source_piece.get("piece_texture") as Texture2D
	)
...
func _on_piece_dropped(piece: Node) -> void:
	...
	var selected_slot: Node = _find_available_slot(
	...
	var piece_was_placed: bool = bool(
		selected_slot.call("place_piece", piece)
	)
