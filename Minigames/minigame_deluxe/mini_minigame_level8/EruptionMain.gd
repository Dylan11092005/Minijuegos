...
# Qué objeto arrastrable corresponde a qué overlay (ya puesto en el niño,
# pero invisible hasta que se coloque bien).
const PARES_OBJETOS := {
	"FaceMask": "KidFaceMask",
	"Glasses": "KidGlasses",
	"Hat": "KidHat"
}

# Qué tan cerca (en píxeles) hay que soltar cada objeto de su posición
# correcta para que cuente como acierto. Más grande = más fácil/permisivo.
# Si algún objeto necesita más o menos tolerancia que el resto, agregalo
# acá con su propio valor (ej: "Glasses": 90.0); si no está listado, se
# usa SNAP_RADIUS_DEFAULT.
const SNAP_RADIUS_DEFAULT := 110.0
...
# =========================================================
# ARMADO DE LOS OBJETOS ARRASTRABLES
# =========================================================
...
func _update_meta_label():
	if not meta_label:
		return
	meta_label.text = "Protección colocada: %d / %d" % [placed_count, target_count]
...
func _setup_draggable_items():
	...
	for nombre_objeto in PARES_OBJETOS.keys():
		...
		# El objeto tiene que soltarse cerca de donde va su versión en el niño.
...
func _start_global_timer():
	...
	if timer_ui.has_method("iniciar"):
		timer_ui.iniciar(GAME_TIME, "Tiempo restante", "para vestir al niño")
	else:
...
# =========================================================
# RESULTADO DE COLOCAR UN OBJETO
# =========================================================
